...
if ($allow_active_news) {
    ...
    while ($row = $db->get_row($sql_result)) {
        ...
        if ($title_arr[0]) $row['title_0'] = $title_arr[0];
        else            $row['title_0'] = $row['title'];
        ...
        if ($row['price'] == 1) {
            ...
            $tpl->set('[price]', '');
            $tpl->set('[/price]', '');
            $tpl->set_block("'\\[price-1\\](.*?)\\[/price-1\\]'si", "");
        }


        $row['_autoName'] = str_replace('_', '-', totranslit($row['autoName'], true));
        $row['_dvigNumber'] = str_replace('_', '-', totranslit($row['dvigNumber'], true));
        $row['_dvigPV'] = str_replace('_', '-', totranslit($row['dvigPV'], true));

        foreach ($row as $field => $val) {
            if ($field != 'price') $tpl->set('{' . strtolower($field) . '}', $val);
        }
        ...
        $news_find = ['{comments-num}' => number_format($row['comm_num'], 0, ',', ' '),
...
//$row['category'] AND
        ...
        // var_dump( $catsByAltName[$mainCat]['icon']);
        // exit();
        if ($catsByAltName[$mainCat]['icon']) {

            $tpl->set('{category-icon}', $catsByAltName[$mainCat]['icon']);

        } else {

            $tpl->set('{category-icon}', "{THEME}/img/item.png");
        ...
        if ($view_template == "rss") {
            ...
            if ($config['rss_format'] != 1) {
                ...
                $row['short_story'] = trim(htmlspecialchars(strip_tags(str_replace(["<br>",
                    "<br />"], " ", $row['short_story'])), ENT_QUOTES, $config['charset']));
            ...
            $row['full_story'] = trim(htmlspecialchars(strip_tags(str_replace(["<br>",
                "<br />"], " ", $row['full_story'])), ENT_QUOTES, $config['charset']));
    ...
    if (stripos($tpl->result['content'], "[hide") !== false) {
        ...
        $tpl->result['content'] = preg_replace_callback("#\[hide(.*?)\](.+?)\[/hide\]#is", function ($matches) use ($member_id, $user_group, $lang) {
    ...
    if (!$view_template AND $count_all AND $config['news_navigation'] AND $news_found) {
        ...
        // Previous link
        //----------------------------------
        $url_page = str_replace('turubiny', $mainCat, $url_page);
        ...
        if (!$no_prev OR !$no_next) {
            ...
            switch ($config['news_navigation']) {
                ...
                default :
                    ...
                    // $tpl->result['navi'] .= $tpl->result['navi'];
                    break;
