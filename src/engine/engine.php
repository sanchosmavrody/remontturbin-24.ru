...
switch ($do) {
    ...
    case "catalog" :
            include(ENGINE_DIR . '/modules/center/catalog.php');
        break;

    case "catalog_auto" :
        include(ENGINE_DIR . '/modules/catalog_auto.php');
        break;
    ...
    default :
        ...
        $sql_select = "SELECT p.id, p.autor, p.date, p.short_story, CHAR_LENGTH(p.full_story) as full_story, 
p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, 
p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.editor, e.reason 
FROM " . PREFIX . "_post p {$cat_join}
LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) 
WHERE {$stop_list}approve=1 AND allow_main=1" . $where_date . " 
ORDER BY " . $fixed . $news_sort_by . " " . $news_direction_by . " 
LIMIT " . $cstart . "," . $config['news_number'];
        ...
        if ($do == "cat" and $category != '' and $subaction == '') {
            ...
            $encoded_tag = rawurlencode(str_replace(["&#039;",
                "&quot;",
                "&amp;"], ["'",
                '"',
                "&"], $tag));
            ...
            if ($xfname) {
                ...
                $url_page = $config['http_home_url'] . "xfsearch/{$xfname}/" . rawurlencode(str_replace(["&#039;",
                        "&quot;"], ["'",
                        '"'], $xf));
                $user_query = "do=xfsearch&amp;xfname=" . $xfname . "&amp;xf=" . rawurlencode(str_replace(["&#039;",
                        "&quot;"], ["'",
                        '"'], $xf));
                ...
                $url_page = $config['http_home_url'] . "xfsearch/" . rawurlencode(str_replace(["&#039;",
                        "&quot;"], ["'",
                        '"'], $xf));
                $user_query = "do=xfsearch&amp;xf=" . rawurlencode(str_replace(["&#039;",
                        "&quot;"], ["'",
                        '"'], $xf));
...
if ($do == "cat" and $category != '' and $subaction == '') {
    ...
    if ($subCatMarka) {
        $metatags['header_title'] = <<<TEXT
Купить {$catsByAltName[$mainCat]['name_r']} для {$catsByAltName[$subCatMarka]['name']} в Москве - интернет-магазин EE-TurboSklad
TEXT;
        $metatags['description'] = <<<TEXT
Продажа {$catsByAltName[$mainCat]['name_v']} для всех моделей {$catsByAltName[$subCatMarka]['name']}. Выгодные цены, гарантия на комплектующие в интернет-магазине EE-TurboSklad.
TEXT;
    }
    if ($subCatModel) {
        $metatags['header_title'] = <<<TEXT
Купить {$catsByAltName[$mainCat]['name_r']} {$catsByAltName[$subCatMarka]['name']} {$catsByAltName[$subCatModel]['name']} в Москве - интернет-магазин EE-TurboSklad
TEXT;
        $metatags['description'] = <<<TEXT
Продажа {$catsByAltName[$mainCat]['name_v']} для {$catsByAltName[$subCatMarka]['name']} {$catsByAltName[$subCatModel]['name']} по выгодным ценам. В интернет-магазине EE-TurboSklad огромный выбор комплектующих.
TEXT;
    }

    if ($mainCat AND !$subCatMarka AND !$subCatModel) {
        $metatags['header_title'] = $catsByAltName[$mainCat]['metatitle'];
        $metatags['description'] = $catsByAltName[$mainCat]['fulldescr'];
    }


    foreach ($row as $field => $val) {
        $metatags['header_title'] = str_replace('{' . $field . '}', $val, $metatags['header_title']);
        $metatags['description'] = str_replace('{' . $field . '}', $val, $metatags['description']);
    }
...
elseif ($catalog != "") {
    ...
    if (($subaction != '' or $newsid != '') and $news_found) {
        include(ENGINE_DIR . '/modules/center/SeoElements.php');
        $titl_e = $metatags['title'];
...
$metatags['title'] = str_replace(["{",
    "}",
    "[",
    "]"], "", $metatags['title']);
$metatags['description'] = str_replace(["{",
    "}",
    "[",
    "]"], "", $metatags['description']);
$metatags['keywords'] = str_replace(["{",
    "}",
    "[",
    "]"], "", $metatags['keywords']);
...
HTML;
if (($do == 'catalog' OR $do == 'main_city') OR isset($_REQUEST['newsid'])
) {
...
if ($config['speedbar'] AND !$view_template) {
    ...
    $s_navigation = "<li class='breads__item'><a href=\"{$config['http_home_url']}\" >" . $config['short_title'] . "</a></li>";
    ...
    if (intval($category_id)) {
        ...
        $catSped = get_breadcrumbcategories(intval($category_id), '/', $last_link);


        $catSped = <<<HTML
<li class='breads__item'>{$catSped}</li>
HTML;


        if (substr($catSped, 0, 2) !== '/') {
            $s_navigation .= "" . $catSped;
        } else {
            $s_navigation .= "" . $catSped;
        }
        ...
        $nam_e = str_replace('|', ' ', $nam_e);
        $s_navigation .= "/" . $nam_e . '</li>';
    }

    if ($titl_e) {
        $titl_e = str_replace('|', ' ', $titl_e);
        $s_navigation .= "<li class='breads__item'>" . $titl_e;
    ...
    $tpl->result['content'] = str_replace("{speedbar}", '' . stripslashes($s_navigation) . '', $tpl->result['content']);
