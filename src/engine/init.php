    ...
    if (!is_array($custom_metatags)) {
        ...
        while ($row = $db->get_row()) {
            ...
            if (strpos($row['url'], "*") !== false) {
                ...
                $custom_metatags['regex'][$row['url']] = ['title'            => $row['title'],
                ...
                $custom_metatags['simple'][$row['url']] = ['title'            => $row['title'],
                                                           'description'      => $row['description'],
                                                           'keywords'         => $row['keywords'],
                                                           'page_title'       => $row['page_title'],
                                                           'page_description' => stripslashes($row['page_description'])];
...
if ($config['category_newscount']) {
    ...
    if ($news_count_in_array) {
        ...
        if (!is_array($news_count_in_array)) $news_count_in_array = [];
...
if (!is_array($banned_info)) {
    ...
    while ($row = $db->get_row()) {
        ...
        if ($row['users_id']) {
            ...
            $banned_info['users_id'][$row['users_id']] = ['users_id' => $row['users_id'],
            ...
            if (count(explode(".", $row['ip'])) == 4 OR filter_var($row['ip'], FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) OR strpos($row['ip'], ":") !== false) $banned_info['ip'][$row['ip']] = ['ip'    => $row['ip'],
                                                                                                                                                                                             'descr' => stripslashes($row['descr']),
                                                                                                                                                                                             'date'  => $row['date']];
            elseif (strpos($row['ip'], "@") !== false) $banned_info['email'][$row['ip']] = ['email' => $row['ip'],
                                                                                            'descr' => stripslashes($row['descr']),
                                                                                            'date'  => $row['date']];
            else $banned_info['name'][$row['ip']] = ['name'  => $row['ip'],
                                                     'descr' => stripslashes($row['descr']),
                                                     'date'  => $row['date']];
...
if ($config['allow_social'] AND $config['allow_registration'] AND !$is_logged) {
    ...
    if ($social_config['vk']) {
        ...
        $social_params = ['client_id'     => $social_config['vkid'],
    ...
    if ($social_config['od']) {
        ...
        $social_params = ['client_id'     => $social_config['odid'],
    ...
    if ($social_config['fc']) {
        ...
        $social_params = ['client_id'     => $social_config['fcid'],
    ...
    if ($social_config['google']) {
        ...
        $social_params = ['client_id'     => $social_config['googleid'],
    ...
    if ($social_config['mailru']) {
        ...
        $social_params = ['client_id'     => $social_config['mailruid'],
    ...
    if ($social_config['yandex']) {
        ...
        $social_params = ['client_id'     => $social_config['yandexid'],
